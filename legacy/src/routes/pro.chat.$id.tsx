...
import { useStored, KEYS, STUDENTS, type ChatMessage, type BlockedUser, type SafetyReport } from "@/lib/storage";

export const Route = createFileRoute("/pro/chat/$id")({
  component: ProChat,
  loader: ({ params }) => {
  ...
  head: ({ loaderData }) => ({
    meta: [{ title: `Chat com ${loaderData?.student.name ?? "aluno"} — MyOwnTraining` }],
  }),
});

function ProChat() {
  const { student } = Route.useLoaderData() as { student: typeof STUDENTS[number] };
  const nav = useNavigate();
  const [allChats, setChats] = useStored<Record<string, ChatMessage[]>>(KEYS.proChats, {});
  ...
  const isBlocked = blocked.some((b) => b.targetId === student.id);
  const messages = allChats[student.id] ?? [
    { id: "m0", from: "them", text: `Olá professor! Obrigado pelo treino.`, ts: Date.now() - 60000 },
  ];
  ...
  const blockUser = () => {
    setBlocked([{ id: `b-${Date.now()}`, blockedBy: "trainer", targetId: student.id, date: Date.now() }, ...blocked]);
    toast.success(`${student.name} bloqueado`); setMenuOpen(false); nav({ to: "/pro/history" });
  };
  const reportUser = () => {
    setReports((prev) => [{ id: `r-${Date.now()}`, bookingId: "", reporter: "trainer", targetId: student.id, reason: "Comportamento no chat", date: Date.now() }, ...prev]);
    toast.success("Reporte enviado"); setMenuOpen(false);
  };
  ...
  const send = (e: React.FormEvent) => {
    ...
    setChats((prev) => ({ ...prev, [student.id]: [...(prev[student.id] ?? messages), mine] }));
    setText("");
  };
    ...
    <div className="flex min-h-screen flex-col bg-background">
      <header className="sticky top-0 z-10 border-b border-border bg-background/95 backdrop-blur">
        <div className="mx-auto flex max-w-md items-center gap-3 px-4 py-3">
          <Link to="/pro/history" className="flex h-9 w-9 items-center justify-center rounded-full bg-muted">
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <img src={student.photo} alt="" className="h-10 w-10 rounded-full bg-muted" />
            ...
            {menuOpen && (
              <div className="absolute right-0 top-11 z-20 w-48 overflow-hidden rounded-xl border border-border bg-card shadow-lg">
                <button onClick={reportUser} className="flex w-full items-center gap-2 px-3 py-2.5 text-left text-xs hover:bg-muted">
                  <Flag className="h-3.5 w-3.5" /> Reportar aluno
                </button>
                {!isBlocked && (
                  <button onClick={blockUser} className="flex w-full items-center gap-2 border-t border-border px-3 py-2.5 text-left text-xs text-destructive hover:bg-muted">
                    <Shield className="h-3.5 w-3.5" /> Bloquear aluno
                  </button>
                )}
              </div>
            )}
