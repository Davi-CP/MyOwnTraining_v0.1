...
import {
  useStored, KEYS, pushNotification,
  type SupportTicket, type SupportTicketType,
} from "@/lib/storage";
import { TRAINERS } from "@/lib/mock-data";
import { toast } from "sonner";

export const Route = createFileRoute("/pro/support")({
  component: SupportPage,
  head: () => ({ meta: [{ title: "Suporte — MyOwnTraining" }] }),
});

const TYPES: SupportTicketType[] = ["Pagamento", "Cliente", "Agendamento", "Reembolso", "Comportamento inadequado", "Outro"];

function SupportPage() {
  const me = TRAINERS[0];
  const [tickets, setTickets] = useStored<SupportTicket[]>(KEYS.supportTickets, []);
  const [type, setType] = useState<SupportTicketType>("Pagamento");
  const [description, setDescription] = useState("");
  const [bookingId, setBookingId] = useState("");
  const [image, setImage] = useState<string | undefined>();

  const myTickets = tickets.filter((t) => t.trainerId === me.id);
  ...
  const submit = () => {
    ...
    const ticket: SupportTicket = {
      id: `tk-${Date.now()}`,
      trainerId: me.id, trainerName: me.name,
    ...
    pushNotification({ audience: "trainer", title: "Chamado enviado", body: "Nossa equipe responderá em breve." });
      ...
      <div className="mx-auto max-w-md px-5 pt-6">
        ...
        <div className="space-y-3 rounded-2xl bg-card p-4 shadow-[var(--shadow-soft)]">
          <label className="block">
            ...
            <select value={type} onChange={(e) => setType(e.target.value as SupportTicketType)}
          ...
          <label className="block">
            ...
            <Input value={bookingId} onChange={(e) => setBookingId(e.target.value)} placeholder="Ex: b1, s-pro-1..." className="h-11 rounded-xl" />
          </label>
          <label className="block">
